/* Version: "367f130" 2019-11-01 11:01:47 +0200 */
/* Version:  v0.2     2020-30-01 */
/*
...
/* sensor names */
#define KX022_SENSOR_NAME "kx022"
#define KX023_SENSOR_NAME "kx023"
#define KX124_SENSOR_NAME "kx124"
#define KX123_SENSOR_NAME "kx123"
#define KX122_SENSOR_NAME "kx122"
#define KX112_SENSOR_NAME "kx112"

/* input dev names */
#define KX022_INPUT_DEV_NAME "accelerometer"
#define KX023_INPUT_DEV_NAME "accelerometer"
#define KX124_INPUT_DEV_NAME "accelerometer"
#define KX123_INPUT_DEV_NAME "accelerometer"
#define KX122_INPUT_DEV_NAME "accelerometer"
#define KX112_INPUT_DEV_NAME "accelerometer"
...
/* power (milliAmps) is same for kx02x, 112 and 122 */
...
static char* kx122_get_sensor_name(u8 wai)
{
	char* name = NULL;
	
	if (wai == KX022_WHO_AM_I_WIA_ID)
		name = KX022_SENSOR_NAME;
	else if (wai == KX023_WHO_AM_I_WIA_ID)
		name = KX023_SENSOR_NAME;
	else if (wai == KX123_WHO_AM_I_WIA_ID)
		name = KX123_SENSOR_NAME;
	else if (wai == KX122_WHO_AM_I_WIA_ID)
		name = KX122_SENSOR_NAME;
	else if (wai == KX112_WHO_AM_I_WIA_ID)
		name = KX112_SENSOR_NAME;
	else if (wai == KX124_WHO_AM_I_WIA_ID)
		name = KX124_SENSOR_NAME;
...
static int kx122_hw_detect(struct kx122_data *sdata)
{
	...
	wai = kx122_reg_read_byte(sdata, KX122_WHO_AM_I);
	if (wai < 0)
		return wai;

	__debug("%s WHO_AM_I_WIA_ID = 0x%x\n", __FUNCTION__, wai);

	name = kx122_get_sensor_name(wai);

	if (name == NULL)
		return -ENODEV;

	/* supported sensor found*/
	sdata->wai = wai;

	dev_info(&sdata->client->dev, "Kionix %s detected\n", name);
...
static irqreturn_t kx122_irq_handler(int irq, void *dev)
{
	...
	//__debug("%s - INS2 status=0x%x\n", __FUNCTION__, status);

	if (status < 0)
		return IRQ_HANDLED;

	if (status == 0) {
		dev_err(&sdata->client->dev, "%s - INS2 status ZERO !\n",
			__FUNCTION__);
		return IRQ_HANDLED;
	}
...
#define KX122_FIFO_SAMPLES_SIZE_BYTES 6
#define KX122_FIFO_MAX_SAMPLES (KX122_FIFO_SIZE_BYTES/KX122_FIFO_SAMPLES_SIZE_BYTES)
/* Proper WMI configuration depends on following */
/* - FIFO sample loss may occur if high ORD, low I2C clock, large FIFO wmi */
/*   SW workaround is implemented to estimate FIFO sample loss. */
/* - kernel/Android HAL sample loss may occur if input dev buffer too small */

#define KX122_FIFO_MAX_WMI_TH 80
#define KX122_INPUT_DEV_EVENTS_NUM 120

#define KX022_FIFO_MAX_WMI_TH 40
...
static int kx122_fifo_calculate_wmi(struct kx122_data *sdata)
{
	...
	/* set max threshold based on used sensor */
	if (sdata->wai == KX022_WHO_AM_I_WIA_ID || sdata->wai == KX023_WHO_AM_I_WIA_ID) {
		if (threshold > KX022_FIFO_MAX_WMI_TH)
...
static int kx122_fifo_enable(struct kx122_data *sdata)
{
	...
	if (sdata->wai == KX022_WHO_AM_I_WIA_ID || sdata->wai == KX023_WHO_AM_I_WIA_ID) {
		/* KX022/KX023 watermark threshold is 7bits */
		reg_val = 0x007f & sdata->fifo_wmi_threshold;
	} else {
...
static int kx122_fifo_report_data(struct kx122_data *sdata)
{
	...
	if (sdata->wai == KX022_WHO_AM_I_WIA_ID || sdata->wai == KX023_WHO_AM_I_WIA_ID) {
		/* fifo bytes info in lowest 8bits */
		fifo_bytes = 0x00FF & fifo_bytes;
		fifo_sample_set_count = fifo_bytes/KX122_FIFO_SAMPLES_SIZE_BYTES;

		/* safety check fifo sample count */
		if (fifo_sample_set_count > KX022_FIFO_MAX_WMI_TH) {
			fifo_sample_set_count = KX022_FIFO_MAX_WMI_TH;
		}
	} else {
	...
	__debug("%s - wmi bytes %d, fifo bytes %d, fifo samples %d \n",
		__FUNCTION__,
		sdata->fifo_wmi_threshold*KX122_FIFO_SAMPLES_SIZE_BYTES,
		fifo_bytes,
...
static int kx122_input_dev_register(struct kx122_data *sdata)
{
	...
	if (sdata->wai == KX022_WHO_AM_I_WIA_ID)
		sdata->accel_input_dev->name = KX022_INPUT_DEV_NAME;
	else if (sdata->wai == KX023_WHO_AM_I_WIA_ID)
		sdata->accel_input_dev->name = KX023_INPUT_DEV_NAME;
	else if (sdata->wai == KX112_WHO_AM_I_WIA_ID)
		sdata->accel_input_dev->name = KX112_INPUT_DEV_NAME;
	else if (sdata->wai == KX123_WHO_AM_I_WIA_ID)
		sdata->accel_input_dev->name = KX123_INPUT_DEV_NAME;
	else if (sdata->wai == KX124_WHO_AM_I_WIA_ID)
		sdata->accel_input_dev->name = KX124_INPUT_DEV_NAME;
	else
...
static void kx122_input_report_xyz(struct input_dev *dev, int *xyz, ktime_t ts)
{
	...
	__debug("kx122 x,y,z,ts:, %d, %d, %d, %lld \n", xyz[0], xyz[1], xyz[2],
		ktime_to_ms(ts) );
#endif
...
#define KX022_FIFO_MAX_EVENT_COUNT KX022_FIFO_MAX_WMI_TH
#define KX022_FIFO_RESERVED_EVENT_COUNT KX022_FIFO_MAX_WMI_TH
...
int kx122_sensors_cdev_register(struct kx122_data *sdata)
{
	...
	/* sensor name */
	if (sdata->wai == KX022_WHO_AM_I_WIA_ID)
		sdata->accel_cdev.name = KX022_SENSOR_NAME;
	else if (sdata->wai == KX023_WHO_AM_I_WIA_ID)
		sdata->accel_cdev.name = KX023_SENSOR_NAME;
	else if (sdata->wai == KX112_WHO_AM_I_WIA_ID)
	...
	if (sdata->wai == KX022_WHO_AM_I_WIA_ID || sdata->wai == KX023_WHO_AM_I_WIA_ID) {
		sdata->accel_cdev.fifo_reserved_event_count = KX022_FIFO_RESERVED_EVENT_COUNT,
		sdata->accel_cdev.fifo_max_event_count = KX022_FIFO_MAX_EVENT_COUNT;
		/* max fifo size in time (1Hz * fifo events) */
		sdata->accel_cdev.max_delay = MAX_POLL_RATE_MS * KX022_FIFO_MAX_EVENT_COUNT;
	}
	else {
