		...
		size_t buf_size, u8 *ic_buf, size_t ic_buf_size)
{
	...
	while (pbuf <= (buf + buf_size)) {
		if (i >= CY_MAX_CONFIG_BYTES) {
			htc_dev_err(dev, "%s: %s size=%d max=%d\n", __func__,
					"Max cmd size exceeded", i,
					CY_MAX_CONFIG_BYTES);
			return -EINVAL;
		}
		if (i >= ic_buf_size) {
			htc_dev_err(dev, "%s: %s size=%d buf_size=%zu\n", __func__,
		...
		if ((last == ',') && (*pbuf == ',')) {
			htc_dev_err(dev, "%s: %s \",,\" not allowed.\n", __func__,
					"Invalid data format.");
			return -EINVAL;
		}
		...
		if (ret < 0) {
			htc_dev_err(dev, "%s: %s '%s' %s%s i=%d r=%d\n", __func__,
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	length = cyttsp5_ic_parse_input(dev, buf, size, dad->ic_buf,
			CY_MAX_PRBUF_SIZE);
	if (length <= 0) {
		htc_dev_err(dev, "%s: %s Group Data store\n", __func__,
				"Malformed input for");
		goto exit;
	}
	...
	struct configuration *configuration)
{
	...
	if (tx_num != dad->si->sensing_conf_data.tx_num) {
		htc_dev_err(dev, "%s: TX number mismatch!\n", __func__);
		ret = -EINVAL;
	}

	if (rx_num != dad->si->sensing_conf_data.rx_num) {
		htc_dev_err(dev, "%s: RX number mismatch!\n", __func__);
		ret = -EINVAL;
	}

	if (button_num != dad->si->num_btns) {
		htc_dev_err(dev, "%s: Button number mismatch!\n", __func__);
		ret = -EINVAL;
	}
	...
	struct result *result, bool *pass, int test_item)
{
	...
	for (i = 0; i < sensor_num; i++) {
		...
		|| (cm_sensor_data[i] > cm_sensor_max)) {
			htc_dev_err(dev, "%s: Sensor[%d,%d]:%d (%d,%d)\n",
	...
	*/
	...
	if (cm_sensor_calibration <
		...
		htc_dev_err(dev, "%s: Cm_cal:%d (%d,%d)\n",
	...
	struct cmcp_data *cmcp_info)
{
	...
	dev_dbg(dev, "%s tx_num=%d", __func__, tx_num); //htc
	dev_dbg(dev, "%s rx_num=%d", __func__, rx_num); //htc
	...
	if (rc) {
		htc_dev_err(dev, "short test not supported");
		goto exit;
	}
	if (dad->ic_buf[1] != 0)
	...
	if (rc) {
		htc_dev_err(dev, "Get CM Panel not supported");
		goto exit;
	}
	...
	if (rc) {
		htc_dev_err(dev, "Get CP Panel not supported");
		goto exit;
	}
	...
	if (rc) {
		htc_dev_err(dev, "Get CM BTN not supported");
		goto exit;
	}
	if (cm_btn_data != NULL) {
		for (i = 0; i < btn_num; i++) {
	...
	if (rc) {
		htc_dev_err(dev, "Get CP BTN not supported");
		goto exit;
	}
	if (cp_btn_data != NULL) {
		for (i = 0; i < btn_num; i++) {
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (dad->builtin_cmcp_threshold_status < 0) {
		htc_dev_err(dev, "%s: No cmcp threshold file.\n", __func__);
		no_builtin_file = 1;
		mutex_unlock(&dad->sysfs_lock);
		goto start_testing;
	}
...
start_testing:
	...
	if (rc)
		htc_dev_err(dev, "start watchdog failed");
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (length <= 0 || length > 3) {
		htc_dev_err(dev, "%s: Input format error!\n", __func__);
	...
	int no_builtin_file)
{
	...
	if (configuration == NULL)
		htc_dev_err(dev, "config is NULL");
	if (result == NULL)
		htc_dev_err(dev, "result is NULL");
	if (cmcp_info == NULL)
		htc_dev_err(dev, "cmcp_info is NULL");
		...
		char __user *buf, size_t count, loff_t *ppos)
{
	...
	if (test_executed)
		...
		htc_dev_err(dev, "%s: No test result available!\n", __func__);
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (value == 1)
		...
		if (dad->cmcp_threshold_size == 0) {
			htc_dev_err(dev, "%s: No cmcp threshold data\n", __func__);
			goto exit_free;
		}
...
static int cyttsp5_suspend_scan_cmd_(struct device *dev)
{
	...
	rc = cmd->nonhid_cmd->suspend_scanning(dev, 0);
	if (rc < 0)
		htc_dev_err(dev, "%s: Suspend scan failed r=%d\n",
			__func__, rc);
	return rc;
}
...
static int cyttsp5_resume_scan_cmd_(struct device *dev)
{
	...
	rc = cmd->nonhid_cmd->resume_scanning(dev, 0);
	if (rc < 0)
		htc_dev_err(dev, "%s: Resume scan failed r=%d\n",
			__func__, rc);
	return rc;
}
...
static int cyttsp5_exec_scan_cmd_(struct device *dev)
{
	...
	if (rc < 0)
		htc_dev_err(dev, "%s: Heatmap start scan failed r=%d\n",
			__func__, rc);
	return rc;
}
		...
		u16 *actual_read_len, u8 *return_buf)
{
	...
	rc = cmd->nonhid_cmd->retrieve_panel_scan(dev, 0, read_offset,
			...
			return_buf);
	if (rc < 0)
		htc_dev_err(dev, "%s: Retrieve scan data failed r=%d\n",
				__func__, rc);
	return rc;
}
		...
		u16 *actual_read_len, u8 *data)
{
	...
	if (rc < 0)
		htc_dev_err(dev, "%s: Get data structure failed r=%d\n",
				__func__, rc);
	return rc;
}
		...
		u8 *results_available)
{
	...
	rc = cmd->nonhid_cmd->run_selftest(dev, 0, test_id,
			...
			results_available);
	if (rc < 0)
		htc_dev_err(dev, "%s: Run self test failed r=%d\n",
				__func__, rc);
	return rc;
}
		...
		u16 *actual_read_len, u8 *data)
{
	...
	if (rc < 0)
		htc_dev_err(dev, "%s: Get self test result failed r=%d\n",
				__func__, rc);
	return rc;
}
		...
		bool get_result_on_pass)
{
	...
	if (rc < 0) {
		htc_dev_err(dev, "%s: Error on suspend scan r=%d\n",
				__func__, rc);
	...
	if (rc < 0) {
		htc_dev_err(dev, "%s: Error on run self test for test_id:%d r=%d\n",
				__func__, test_id, rc);
		goto resume_scan;
	}
	...
	if (rc < 0) {
		htc_dev_err(dev, "%s: Error on get self test result r=%d\n",
				__func__, rc);
		goto resume_scan;
	}
...
put_pm_runtime:
	...
	if(test_id == CY_ST_ID_CM_PANEL) {
		size = htc_prepare_print_buffer(dev, status, dad->ic_buf, length, buf, buf_len,
			6, eHTC_MFG_CM_PANEL);
	}
	else if(test_id == CY_ST_ID_CP_PANEL) {
		size = htc_prepare_print_buffer(dev, status, dad->ic_buf, length, buf, buf_len,
			6, eHTC_MFG_CP_PANEL);
	}
	else {
		...
		const char __user *buf, size_t count, loff_t *ppos)
{
	...
	if (length != 1) {
		htc_dev_err(dad->dev, "%s: Malformed input\n", __func__);
		rc = -EINVAL;
		goto exit_unlock;
	}
		...
		size_t count, loff_t *ppos)
{
	...
	if (rc < 0) {
		htc_dev_err(dev, "%s: Error on get data structure r=%d\n",
				__func__, rc);
		goto resume_scan;
	}
		...
		size_t count, loff_t *ppos)
{
	...
	rc = _cyttsp5_calibrate_idacs_cmd(dev, dad->calibrate_sensing_mode,
			&dad->ic_buf[0]);
	if (rc < 0) {
		htc_dev_err(dev, "%s: Error on calibrate idacs r=%d\n",
				__func__, rc);
		goto resume_scan;
	}
	...
	if (dad->calibrate_initialize_baselines) {
		...
		rc = _cyttsp5_initialize_baselines_cmd(dev, CY_IB_SM_MUTCAP |
				...
				&dad->ic_buf[length]);
		if (rc < 0) {
			htc_dev_err(dev, "%s: Error on initialize baselines r=%d\n",
					__func__, rc);
			goto resume_scan;
		}
...
static int cyttsp5_setup_sysfs(struct device *dev)
{
	...
	if (rc) {
		htc_dev_err(dev, "%s: Error, could not create command\n",
				__func__);
		goto exit;
	}
	...
	if (rc) {
		htc_dev_err(dev, "%s: Error, could not create status\n",
				__func__);
	...
	if (rc) {
		htc_dev_err(dev, "%s: Error, could not create response\n",
				__func__);
	...
	if (IS_ERR_OR_NULL(dad->base_dentry)) {
		htc_dev_err(dev, "%s: Error, could not create base directory\n",
				__func__);
	...
	if (IS_ERR_OR_NULL(dad->mfg_test_dentry)) {
		htc_dev_err(dev, "%s: Error, could not create mfg_test directory\n",
				__func__);
	...
	if (rc) {
		htc_dev_err(dev, "%s: Error, could not create cmcp_test\n",
				__func__);
		goto unregister_base_dir;
	}
	...
	if (rc) {
		htc_dev_err(dev, "%s: Error, could not create cmcp_thresold_loading\n",
				__func__);
	...
	if (rc) {
		htc_dev_err(dev, "%s: Error, could not create cmcp_thresold_data\n",
				__func__);
		...
		void *context)
{
	...
	if (!fw->data || !fw->size) {
		htc_dev_err(dev, "%s: Invalid builtin cmcp threshold file\n",
		__func__);
		goto exit;
	}
	...
	struct work_struct *cmcp_threshold_update)
{
	...
	if (retval < 0) {
		htc_dev_err(dev, "%s: Fail request cmcp threshold class file load\n",
			__func__);
		goto exit;
	}
...
static int cyttsp5_device_access_probe(struct device *dev, void **data)
{
	...
	*data = dad;
	...
	if (dad->si) {
		...
		htc_dev_err(dev, "%s: Fail get sysinfo pointer from core p=%p\n",
				__func__, dad->si);
		cmd->subscribe_attention(dev, CY_ATTEN_STARTUP,
			CYTTSP5_DEVICE_ACCESS_NAME,
			cyttsp5_setup_sysfs_attention, 0);
	}
	dev_info(dev, "%s: cyttsp5_device_access_probe end, then schedule work\n", __func__);
