...
#define CYTTSP5_NEW_FPC_BASE_FW_VERSION   50
...
struct cyttsp5_tp_info {
    ...
    u8 panel_id;
    u8 *module_name;
    u8 flag;    //0: old fpc, 1:new fpc , for  separate firmware
};
		...
		u32 fw_ver_new, u32 fw_revctrl_new)
{
	...
	{
		//cd->panel_id
		size = sizeof(hw_tp_info)/sizeof(struct cyttsp5_tp_info);
		...
		//used for check burn bad version fw,
		if( ((ld->si->cydata.fw_ver_conf >= CYTTSP5_NEW_FPC_BASE_FW_VERSION) && (flag == 0))
			|| ((ld->si->cydata.fw_ver_conf < CYTTSP5_NEW_FPC_BASE_FW_VERSION) && (flag == 1))){
			tp_log_info( "%s: Image flag check is error, will upgrade\n", __func__);
			return 1;
		}
	}
...
Parameters    :  panel_name
                 ...
                 panel_id
Return        :
Description   :  get panel name by panel id for new FPC (T2-10 , T2-7)
*****************************************************************/
static unsigned int cyttsp5_get_panel_name_pad(const char *product_name, char *panel_name, unsigned int length,
											unsigned int panel_id)
{
	int index = 0;
	int retval = 0;
	int size = 0;
	if (NULL == product_name || NULL == panel_name || length <= 0) {
		tp_log_err("%s %d:Missing input parameter\n", __func__, __LINE__);
		return 0;
	}
	memset(panel_name, 0, length);
	size = sizeof(hw_tp_info)/sizeof(struct cyttsp5_tp_info);
	for(index = 0; index < size; index++) {
		retval = strncasecmp(hw_tp_info[index].product_name, product_name, strlen(hw_tp_info[index].product_name));
		if(retval == 0 && hw_tp_info[index].panel_id == panel_id){
			strncpy(panel_name, hw_tp_info[index].module_name, length);
			break;
		}
		if(index == (size - 1)){
			strncpy(panel_name, "null", length);
		}
	}
	tp_log_info("%s %d:Panel name is %s\n", __func__, __LINE__, panel_name);
	return strnlen(panel_name, length);
		...
		int fw_size)
{
	...
	wake_lock_timeout(&cd->fw_upgrade_lock, 15 * HZ);
...
static char *generate_firmware_filename(struct device *dev)
{
	...
	if(core_pdata->new_fpc_flag == 1) {
		length = sizeof(hw_tp_info)/sizeof(struct cyttsp5_tp_info);
		for(index = 0; index < length; index++) {
			retval = strncasecmp(hw_tp_info[index].product_name, product_name, strlen(hw_tp_info[index].product_name));
			if(retval == 0 && hw_tp_info[index].panel_id == panel_id){
				if(1 == hw_tp_info[index].flag) {
					snprintf(filename, FILENAME_LEN_MAX, "%s_fw_new.bin", product_name);
				} else {
...
static void cyttsp5_set_app_info(struct device * dev)
{
	...
	panel_id = cyttsp5_get_panel_id(dev);
	if(cd->cpdata->new_fpc_flag == 1) {
		cyttsp5_get_panel_name_pad(cd->cpdata->product_name, panel_name, PANEL_NAME_LEN_MAX - 1, panel_id);
	} else {
	...
	if ((core_pdata->chip_name) && (strcmp(core_pdata->chip_name, "CS448") == 0)) {
		snprintf(touch_info, APP_INFO_VALUE_LENTH - 1,
				...
				cydata->fw_ver_minor,
				cydata->fw_ver_conf);
	} else if (((core_pdata->chip_name) && (strcmp(core_pdata->chip_name, "CYTT21403") == 0)) &&
		((core_pdata->product_name) && (strcmp(core_pdata->product_name, "T27") == 0))) {
		snprintf(touch_info, APP_INFO_VALUE_LENTH - 1,
				"cytt21403_%s_%d.%d.%d",
				panel_name,
				cydata->fw_ver_major,
		...
		struct work_struct *fw_and_config_upgrade)
{
	...
	if( (cd->cpdata->new_fpc_flag == 1) && (cd->cpdata->firmware_broken == 1) ) {
		tp_log_info("%s firmware_broken : %d, first don't set app info\n", __func__, cd->cpdata->firmware_broken);
	} else {
...
#endif
	if(cd->cpdata->new_fpc_flag == 1) {
		tp_log_info("%s firmware_broken : %d\n", __func__, cd->cpdata->firmware_broken);
		if(cd->cpdata->firmware_broken == 1 ) {
			schedule_work(&ld->fw_and_config_upgrade);
			cd->cpdata->firmware_broken = 0;
		}
	}
}
