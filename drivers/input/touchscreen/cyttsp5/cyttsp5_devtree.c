		...
		struct device_node *core_node)
{
	...
	printk("%s: irq_gpio:%d rst_gpio:%d \n" //htc
		"TP_1V8_EN_gpio:%d TP_I2C_SEL_CPU_gpio:%d \n"
		"hid_desc_register:%d level_irq_udelay:%d vendor_id:%d product_id:%d\n"
		"flags:%d easy_wakeup_gesture:%d\n", __func__,
		pdata->irq_gpio, pdata->rst_gpio,//htc
		pdata->TP_1V8_EN_gpio, pdata->TP_I2C_SEL_CPU_gpio, //htc
