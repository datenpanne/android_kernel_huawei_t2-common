...
void mdss_fb_set_backlight(struct msm_fb_data_type *mfd, u32 bkl_lvl)
{
	...
	if ((pdata) && (pdata->set_backlight)) {
		 ...
		 */
		if ((mfd->bl_level_scaled == temp) && (2 == ctrl_pdata->which_product_pad)) {
			mfd->bl_level = bkl_lvl;
		} else {
            if (mfd->bl_level != bkl_lvl || (2 != ctrl_pdata->which_product_pad))
...
static int mdss_fb_config_cabc(struct msm_fb_data_type *mfd,struct msmfb_cabc_config cabc_cfg)
{
	...
	if ((pdata) && (pdata->config_cabc))
	{
		...
		mfd->panel_info->cabc_mode = cabc_cfg.mode;
	}
	else
	{
