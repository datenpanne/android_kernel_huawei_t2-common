...
#define DEFAULT_PARA1			7
#define DEFAULT_PARA2			30983
...
static void mdss_dsi_panel_bklt_dcs_pad(struct mdss_dsi_ctrl_pdata *ctrl, int level)
{
	...
	if (1 == ctrl->which_product_pad)
	{
		...
		cmdreq.cmds_cnt = 1;
		cmdreq.flags = CMD_REQ_COMMIT | CMD_CLK_CTRL | CMD_REQ_HS_MODE;
		if(level==0 && (2 != ctrl->which_product_pad))
			{
	...
	/*Special backlight curve for jordan lcd*/
	if (2 == ctrl->which_product_pad)
	{
		if (level >= 4 && level < 8) {
			if (!strcmp(ctrl->panel_data.panel_info.panel_name,"INX_NT51021_8_1200P_VIDEO")) {
				reg_level = 15 + (level-4)*8;
			} else if (!strcmp(ctrl->panel_data.panel_info.panel_name,"AUO_NT51021_8_1200P_VIDEO")) {
				reg_level = 12 + (level-4)*9;
			}
		} else {
			reg_level = (59*level+5)/10;
		}
...
static char moving_dimming[2] = {0x94, 0x18};

static char cabc_still_dimming_off[2] = {0x95, 0xB0};
static char cabc_still_dimming_on[2] = {0x95, 0x60};
static char cabc_mode_off[2] = {0x90, 0xc0};
static char cabc_mode_on[2] = {0x90, 0x00};

static char chang_page2_index0[2] = {0x83, 0xBB};
static char chang_page2_index1[2] = {0x84, 0x22};

static struct dsi_cmd_desc cabc_off_cmd_pad[] = {
	...
	{{DTYPE_DCS_WRITE1, 0, 0, 0, 1, sizeof(cabc_still_dimming_on)},cabc_still_dimming_on},
	{{DTYPE_DCS_WRITE1, 1, 0, 0, 1, sizeof(cabc_mode_off)},cabc_mode_off},

};

static struct dsi_cmd_desc cabc_moving_cmd_pad[] = {
	{{DTYPE_DCS_WRITE1, 0, 0, 0, 1, sizeof(chang_page2_index0)},chang_page2_index0},
	{{DTYPE_DCS_WRITE1, 0, 0, 0, 1, sizeof(chang_page2_index1)},chang_page2_index1},
	{{DTYPE_DCS_WRITE1, 0, 0, 0, 1, sizeof(cabc_still_dimming_off)},cabc_still_dimming_off},
	{{DTYPE_DCS_WRITE1, 1, 0, 0, 1, sizeof(cabc_mode_on)},cabc_mode_on},
...
static struct dsi_cmd_desc moving_cmd_pad[] = {
	{{DTYPE_DCS_WRITE1, 0, 0, 0, 1, sizeof(chang_page2_index0)},chang_page2_index0},
	{{DTYPE_DCS_WRITE1, 0, 0, 0, 1, sizeof(chang_page2_index1)},chang_page2_index1},
	{{DTYPE_DCS_WRITE1, 1, 0, 0, 1, sizeof(moving_dimming)},moving_dimming},
};



static void mdss_dsi_panel_cabc_mode_pad(struct mdss_dsi_ctrl_pdata *ctrl,bool cabc_flag)
{
	struct dcs_cmd_req cmdreq;
	struct mdss_panel_info *pinfo;
	...
	if(cabc_flag)
	{
		cmdreq.cmds = cabc_moving_cmd_pad;
		cmdreq.cmds_cnt = sizeof(cabc_moving_cmd_pad) / sizeof((cabc_moving_cmd_pad)[0]);
	}
	else
	{
		cmdreq.cmds = cabc_off_cmd_pad;
		cmdreq.cmds_cnt = sizeof(cabc_off_cmd_pad) / sizeof((cabc_off_cmd_pad)[0]);
	}
	cmdreq.flags = CMD_REQ_COMMIT | CMD_CLK_CTRL | CMD_REQ_HS_MODE;
	cmdreq.rlen = 0;
	cmdreq.cb = NULL;
...
static int mdss_dsi_panel_bl_smooth_mode(struct mdss_panel_data *pdata,
							u32 bl_level)
{
	struct mdss_dsi_ctrl_pdata *ctrl_pdata = NULL;
	union power_supply_propval bat_soc_val = {0};
	int bl_switch_point = 0;
	int bl_level_temp = 0;
	if (pdata == NULL) {
	...
	LCD_LOG_INFO("BLK~bat_soc_val.intval= %s,bat_soc_val.intval=%d\n",__func__,bat_soc_val.intval);

	if(bat_soc_val.intval>9)
		{
		bl_switch_point=60;
		switch (bl_level) {                  
			case 6:              
	...
	if(bl_level>bl_switch_point)
	{

		mdss_dsi_panel_cabc_mode_pad(ctrl_pdata,1);
		LCD_LOG_INFO("BLK~cabc moving mode %s,level=%d\n",__func__,bl_level);

	}
	else
	{

		mdss_dsi_panel_cabc_mode_pad(ctrl_pdata,0);
		LCD_LOG_INFO("BLK~cabc off mode %s,level=%d\n",__func__,bl_level);

	}
	bl_level_temp=bl_level;
	return bl_level_temp;
}
...
static void hw_panel_bias_en(struct mdss_panel_data *pdata, int enable)
{
	...
	if (gpio_is_valid(ctrl_pdata->disp_vled_gpio))
	{
		...
		pr_info("%s,%d set en disp_vled_gpio = %d \n",__func__,__LINE__,enable);
	}
	if(enable){
...
int mdss_dsi_panel_reset(struct mdss_panel_data *pdata, int enable)
{
	...
	if (1 == ctrl_pdata->hw_product_pad && 2 == ctrl_pdata->which_product_pad) {
		return rc;
	}
							...
							u32 bl_level)
{
	...
	if (1==ctrl_pdata->hw_product_pad && 0 != bl_level ) {
		if (0 == ctrl_pdata->hw_led_en_flag){
			...
			if (2 == ctrl_pdata->which_product_pad)
			...
			if(1==ctrl_pdata->hw_led_en_flag){
				if (2 == ctrl_pdata->which_product_pad){
					...
					bl_level=mdss_dsi_panel_bl_smooth_mode(pdata,bl_level);
				}
			}
		}
	}
...
static int mdss_dsi_panel_on(struct mdss_panel_data *pdata)
{
	...
	if (ctrl->hw_product_pad && 2 == ctrl->which_product_pad) {
		pinfo->cabc_mode = CABC_MOVING;
	}
	...
	switch(cabc_mode)
	{
		...
		case CABC_OFF:
			if (ctrl_pdata->dsi_panel_cabc_off_cmds.cmd_cnt) {
				...
				LCD_LOG_ERR("%s: Empty cabc mode(CABC_OFF): %d\n", __func__, cabc_mode);
				ret = -1;
			}
			break;
		case CABC_UI:
			if (ctrl_pdata->dsi_panel_cabc_ui_commands.cmd_cnt) {
				...
				LCD_LOG_ERR("%s: Empty cabc mode(CABC_UI): %d\n", __func__, cabc_mode);
				ret = -1;
			}
			break;
		...
		case CABC_MOVING:
			if (ctrl_pdata->dsi_panel_cabc_moving_cmds.cmd_cnt) {
				...
				LCD_LOG_ERR("%s: Empty cabc mode(CABC_MOVING): %d\n", __func__, cabc_mode);
				ret = -1;
			}
			break;
		...
		case CABC_STILL:
			if (ctrl_pdata->dsi_panel_cabc_still_cmds.cmd_cnt) {
				...
				LCD_LOG_ERR("%s: Empty cabc mode(CABC_STILL): %d\n", __func__, cabc_mode);
				ret = -1;
			}
			break;
...
/* Modify JDI tp/lcd power on/off to reduce power consumption */
	...
	rc = of_property_read_u32(np, "huawei,rg-led-flag", &tmp);
	ctrl_pdata->hw_rg_led_flag = (!rc ? tmp : 0);

	if(ctrl_pdata->hw_product_pad){
		if(ctrl_pdata->hw_rg_led_flag == 1){
			pinfo->support_mode |= 0x10001; /*support lcd_comform_mode and rg_led*/
			rc = of_property_read_u32(np, "huawei,rg-led-para1", &tmp);
			pinfo->led_rg_para1 = (!rc ? tmp : DEFAULT_PARA1);
			rc = of_property_read_u32(np, "huawei,rg-led-para2", &tmp);
			pinfo->led_rg_para2 = (!rc ? tmp : DEFAULT_PARA2);
		} else {
